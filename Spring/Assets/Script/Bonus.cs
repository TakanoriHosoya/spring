...
public class Bonus : MonoBehaviour {
	...
	void OnTriggerEnter2D (Collider2D col) {
		if (col.gameObject.name == "Player") {
			...
			enterPlayer.rigidbody2D.velocity = new Vector2 (directionX * Mathf.Abs(enterPlayer.rigidbody2D.velocity.x), enterPlayer.rigidbody2D.velocity.y + addSpeed);
		}
	}
}
